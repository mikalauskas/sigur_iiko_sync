...
async function compareUsers(user1, user2, token, orgId) {
    try {
        ...
        for (const i in user1) {
            ...
            for (const j in user2) {
                const compare = stringSimilarity(fio, user2[j]['NAME']);
                if (compare > 0.95) {
                    // Search user in iiko
                    const res = await iiko.getCustomerInfo(token, orgId, user1[i]['LOGIN']);
                    if (res.message) {
                        // Create user in iiko
                        console.log(res.message);
                        console.log(`Creating user ${user1[i]['LOGIN']}`);
                        const customerId = await iiko.createUser(token, orgId, user1[i]['LOGIN'], user1[i]['NAME'], user1[i]['LAST_NAME'], user1[i]['SECOND_NAME'], user2[j]['CODEKEY']);
