...
function is_parent(incomingUser, CUsers) {
  return (
    ...
    !CUsers.find((CUser) => CUser.phone === incomingUser.customer_phone)
  );
}
...
async function createParent(incomingUser = {}, umedUsers) {
  ...
  if (result) {
    ...
    wazzup.sendMessage({
      phone: paramsData.PHONE_NUMBER.replace(/\+/g, ''),
      text: `Для Вас создана учетная запись в Личном Кабинете UMED. Там вы можете посмотреть расписание, зачетную книжку, оценки своего обучающегося и многое другое! Для входа пройдите по ссылке https://umedcollege.ru/lk Логин: ${paramsData.PHONE_NUMBER} Пароль: ${paramsData.PASSWORD}`,
    });
    return true;
  }
...
async function updateUser(user = {}, umedUsers = [], incomingUser = {}) {
  ...
  /*
    This code aims to automate sending reminders to users who have not logged in 
    for an extended period, ensuring they are aware of their account status and 
    providing them with the necessary information to log in again
  */
  if (forgotToLogin(user.LAST_LOGIN, 90)) {
    ...
    if (wazzupData.length) {
      ...
      const dataIndex = wazzupData.findIndex(
        (el) => el.PHONE_NUMBER === user.PHONE_NUMBER,
      );

      if (dataIndex >= 0) {
        if (forgotToLogin(wazzupData[dataIndex].DATE, 14)) {
          userToUpdate.PASSWORD = incomingUser['UF_APP_PASSWORD'];
          userToUpdate.CONFIRM_PASSWORD = incomingUser['UF_APP_PASSWORD'];
          paramsData.PASSWORD = incomingUser['UF_APP_PASSWORD'];
          paramsData.CONFIRM_PASSWORD = incomingUser['UF_APP_PASSWORD'];
      ...
      const initialData = [
        { PHONE_NUMBER: user.PHONE_NUMBER, DATE: curDate, TYPE: 'forgotToLogin' },
      ];
      await utils.writeJsonData('wazzupMessages.json', initialData);
    }
  }
...
async function updateParent(parent = {}, user = {}, umedUsers = [], incomingUser = {}) {
  ...
  /*
    ...
    providing them with the necessary information to log in again
  */
  if (forgotToLogin(parent.LAST_LOGIN, 90)) {
    const curDate = new Date();
    const wazzupData = await utils.readDataJson('wazzupMessages.json');
    if (wazzupData.length) {
      // check if message was sent at least 14 days ago
      const dataIndex = wazzupData.findIndex(
        (el) => el.PHONE_NUMBER === parent.PHONE_NUMBER,
      );
      ...
      const initialData = [
        { PHONE_NUMBER: parent.PHONE_NUMBER, DATE: curDate, TYPE: 'forgotToLogin' },
      ];
      await utils.writeJsonData('wazzupMessages.json', initialData);
    }
  }
...
const syncUmed = async (access_token = '', CUsers = []) => {
  ...
  for (const incomingUser of CUsers) {
    ...
    const foundDouble = CUsers.filter(
      (el) => el.customer_phone === incomingUser.customer_phone,
    );

    if (foundDouble.length > 1) {
      console.log(`parent: found double ${incomingUser.phone}`);
    }

    if (!is_parent(incomingUser, CUsers)) continue;
    user = findUmedUser(incomingUser, umedUsers);
    const parent = findParentUser(incomingUser, umedUsers);

    if (is_parent(incomingUser, CUsers)) {
      if (utils.isEmpty(parent) && utils.isEmpty(parent.ID)) {
