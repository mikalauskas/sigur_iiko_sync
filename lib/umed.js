...
async function createStudent(incomingUser = {}, umedUsers) {
  ...
  if (result) {
    ...
    if (incomingUser?.PHONE_NUMBER) {
      await utils.delay(Math.floor(Math.random() * (15000 - 10000 + 1)) + 10000);
...
async function updateStudent(incomingUser = {}, umedUsers = []) {
  ...
  if (
    forgotToLogin(parseCustomDate(student.LAST_LOGIN), 45) &&
    forgotToLogin(parseCustomDate(student.TIMESTAMP_X), 50)
  ) {
...
async function updateParent(incomingUser = {}, umedUsers = []) {
  ...
  if (
    forgotToLogin(parseCustomDate(parent.LAST_LOGIN), 45) &&
    forgotToLogin(parseCustomDate(parent.TIMESTAMP_X), 50)
  ) {
    paramsData.PASSWORD = incomingUser.UF_SNILS
      ? incomingUser.UF_SNILS
      : incomingUser.UF_APP_PASSWORD;
