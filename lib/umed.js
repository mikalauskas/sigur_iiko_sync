...
function is_parent(CUser, CUsers) {
  return (
    ...
    CUser.fullname.toLowerCase() !== CUser.customer_name.toLowerCase() &&
    !CUsers.find((user) => CUser.phone === user.phone)
  );
}
...
const findUmedUser = (incomingUser, umedUsers) => {
  ...
  if ((user1 || user2) && user3 && (user1['ID'] || user2['ID']) && user3['ID']) {
    user3['ACTIVE'] = 'N';
    //postData(user2);
  }
...
async function createUser(incomingUser = {}, umedUsers) {
  ...
  if (foundUser) {
    console.log(`found double user with phone ${incomingUser.PHONE_NUMBER}`);
    return false;
  }
...
async function createParent(incomingUser = {}, umedUsers) {
  ...
  if (foundUser) {
    console.log(`found double user with phone ${incomingUser.UF_PARENT_PHONE}`);
    return false;
  }
  ...
  console.log(`create parent ${paramsData.PHONE_NUMBER}`, paramsData);
...
async function updateUser(user = {}, incomingUser = {}) {
  ...
  if (!utils.isObjectEmpty(userToUpdate) && Object.keys(userToUpdate).length) {
    console.log(`update user ${user.ID} ${user.PHONE_NUMBER}`, userToUpdate);
