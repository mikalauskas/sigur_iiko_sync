...
const messageTemplates = (templateName, data) => {
  const templates = {
    createUser: `Для Вас создана учетная запись в Личном Кабинете UMED. Там вы можете посмотреть своё расписание, зачетную книжку, оценки и многое другое! Для входа пройдите по ссылке https://umedcollege.ru/lk Логин: ${data.LOGIN} Пароль: СНИЛС без прочерков и пробелов (11 цифр)`,
    createParent: `Для Вас создана учетная запись в Личном Кабинете UMED. Там вы можете посмотреть расписание, зачетную книжку, оценки студента и многое другое! Для входа пройдите по ссылке https://umedcollege.ru/lk Логин: ${data.LOGIN} Пароль: СНИЛС студента без прочерков и пробелов (11 цифр)`,
  };
...
async function createParent(incomingUser = {}, umedUsers) {
  ...
  if (foundUser) {
    if (umedUsers.find((umedUser) => umedUser.LOGIN === incomingUser.UF_PARENT_EMAIL)) {
      paramsData.LOGIN =
        'parent' + (incomingUser['UF_SNILS'] ?? incomingUser['UF_ID_BITRIX']);
...
async function updateStudent(incomingUser = {}, umedUsers = []) {
  ...
  if (
    ...
    userToUpdate.GROUP_ID = [
      UMED_USER_GROUP_ID,
      UMED_STUDENT_GROUP_ID,
      UMED_CAFE_GROUP_ID,
    ];
  ...
  const doubleStudent = umedUsers.find(
    (umedUser) =>
      ...
      umedUser.UF_PERSON_ID !== incomingUser.UF_PERSON_ID,
  );
...
async function updateParent(incomingUser = {}, umedUsers = []) {
  ...
  if (incomingUser['UF_PARENT_EMAIL']) {
    ...
    umedUsers.find((umedUser) => umedUser.LOGIN === incomingUser.UF_PARENT_EMAIL)
  ) {
    paramsData.LOGIN =
      'parent' + (incomingUser['UF_SNILS'] ?? incomingUser['UF_ID_BITRIX']);
...
async function upgradeToStudent(incomingUser = {}, umedUsers = []) {
  ...
  const student = umedUsers.find(
    (umedUser) =>
      umedUser.LOGIN === incomingUser.LOGIN ||
      umedUser.LOGIN === incomingUser.EMAIL ||
      umedUser.LOGIN === incomingUser.UF_SNILS ||
      umedUser.LOGIN === incomingUser.UF_ID_BITRIX,
  );
  ...
  if (incomingUser['EMAIL']) {
    ...
    delete paramsData.PHONE_NUMBER;
  } else if (umedUsers.find((umedUser) => umedUser.LOGIN === incomingUser.EMAIL)) {
    paramsData.LOGIN = incomingUser['UF_SNILS'] ?? incomingUser['UF_ID_BITRIX'];
...
async function upgradeToParent(incomingUser = {}, umedUsers = []) {
  ...
  const parent = umedUsers.find(
    (umedUser) =>
      ...
      umedUser.LOGIN === 'parent' + incomingUser.UF_SNILS ||
      umedUser.LOGIN === 'parent' + incomingUser.UF_ID_BITRIX,
  );
