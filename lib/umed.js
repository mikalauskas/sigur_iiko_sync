...
function parentsToSync(c1Users, umedUsers) {
  return c1Users.filter((c1User) => {
    const foundUser = umedUsers.find(
      (user) => c1User.customer_name === user.umed_fullname,
    );
...
const syncUmed = async (token, c1Users) => {
  ...
  const parentsToSync1 = c1ToParentsModel(parentsToSync(c1Users, umedUsers));
  ...
  await utils.writeToJson('parentsToSync1.json', parentsToSync1);
  console.log(`Users to sync Umed: ${parentsToSync1.length}`);
