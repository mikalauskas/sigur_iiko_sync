...
const syncUmed = async (access_token = '', CUsers = []) => {
  ...
  for (const incomingUser of CUsers) {
    ...
    if (is_student(incomingUser)) {
      ...
      if (!utils.isEmpty(user && user.ID) && !user.GROUPS_ID.includes('20')) {
        console.log(`umed: remove user ${user.ID} ${user.FULL_NAME}`);
    ...
    if (is_parent(incomingUser, CUsers)) {
      ...
      if (!utils.isEmpty(parent && parent.ID)) {
        console.log(`umed: remove parent ${parent['ID']} ${parent['FULL_NAME']}`);
