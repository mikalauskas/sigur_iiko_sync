...
const messageTemplates = (templateName, data) => {
  const templates = {
    createUser: `Привет, Студент! Для Вас создана учетная запись в Личном Кабинете UMED. Там вы можете посмотреть своё расписание, зачетную книжку, оценки и многое другое! Учебная группа: ${data.UF_COLLEDGE_GROUP}. Для входа пройдите по ссылке https://umedcollege.ru/lk Логин: ${data.LOGIN} Пароль: ${data.PASSWORD ? data.PASSWORD : 'СНИЛС без прочерков и пробелов (11 цифр)'}`,
    createParent: `Привет, Представитель студента! Для Вас создана учетная запись в Личном Кабинете UMED. Там вы можете посмотреть расписание, зачетную книжку, оценки студента и многое другое! Учебная группа: ${data.UF_COLLEDGE_GROUP}. Для входа пройдите по ссылке https://umedcollege.ru/lk Логин: ${data.LOGIN} Пароль: ${data.PASSWORD ? data.PASSWORD : 'СНИЛС студента без прочерков и пробелов (11 цифр)'}`,
    updateUser: `Привет, Студент! Ваша учетная запись в Личном Кабинете UMED была изменена. Учебная группа: ${data.UF_COLLEDGE_GROUP}. Для входа пройдите по ссылке https://umedcollege.ru/lk Логин: ${data.LOGIN} Пароль: ${data.PASSWORD ? data.PASSWORD : 'СНИЛС без прочерков и пробелов (11 цифр)'}`,
    updateParent: `Привет, Представитель студента! Ваша учетная запись в Личном Кабинете UMED была изменена. Учебная группа: ${data.UF_COLLEDGE_GROUP}. Для входа пройдите по ссылке https://umedcollege.ru/lk Логин: ${data.LOGIN} Пароль: ${data.PASSWORD ? data.PASSWORD : 'СНИЛС студента без прочерков и пробелов (11 цифр)'}`,
  };
...
async function createStudent(incomingUser = {}, umedUsers) {
  ...
  const paramsData = {
    ACTIVE: 'Y',
    LOGIN: incomingUser.LOGIN || incomingUser.EMAIL,
    PHONE_NUMBER: incomingUser.LOGIN || null,
  ...
  const phoneExist = umedUsers.find(
    (umedUser) =>
      incomingUser?.LOGIN &&
      (umedUser.PHONE_NUMBER === incomingUser.LOGIN ||
...
async function createParent(incomingUser = {}, umedUsers) {
  ...
  const paramsData = {
    ACTIVE: 'Y',
    LOGIN: incomingUser.UF_PARENT_PHONE || incomingUser.UF_PARENT_EMAIL,
    PHONE_NUMBER: incomingUser.UF_PARENT_PHONE || null,
...
async function updateStudent(incomingUser = {}, umedUsers = []) {
  ...
  if (
    forgotToLogin(parseCustomDate(student.LAST_LOGIN), 30) &&
    forgotToLogin(parseCustomDate(student.TIMESTAMP_X), 35)
  ) {
...
async function updateParent(incomingUser = {}, umedUsers = []) {
  ...
  if (
    forgotToLogin(parseCustomDate(parent.LAST_LOGIN), 30) &&
    forgotToLogin(parseCustomDate(parent.TIMESTAMP_X), 35)
  ) {
    paramsData.PASSWORD = incomingUser.UF_SNILS
      ? incomingUser.UF_SNILS
      : incomingUser.UF_APP_PASSWORD;
