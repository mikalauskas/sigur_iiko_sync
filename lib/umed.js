...
async function createUser(incomingUser = {}, umedUsers) {
  ...
  console.log(incomingUser.PHONE_NUMBER ?? incomingUser.EMAIL);
...
const syncUmed = async (CUsers = []) => {
  ...
  await utils.writeToJson('umedParents.json', umedParents);
  ...
  const { found: studentsToUpdate, notFound: studentsToCreate } = splitById(
    CStudents_valid,
    umedUsers,
    'person_id',
    'UF_PERSON_ID',
  ...
  await utils.writeToJson('studentsToCreate.json', studentsToCreate);

  const { found: parentsToUpdate, notFound: parentsToCreate } = splitById(
    CParents_valid,
    umedUsers,
    'person_id',
    'UF_PARENT_STUDENT_ID',
  );
  await utils.writeToJson('parentsToUpdate.json', parentsToUpdate);
  await utils.writeToJson('parentsToCreate.json', parentsToCreate);

  const { found: abiturientsToStudents } = splitById(
    umedAbiturients,
    CStudents_valid,
    'PHONE_NUMBER',
    'phone',
  );
  await utils.writeToJson('abiturientsToStudents.json', abiturientsToStudents);
  ...
  /* for (const incomingUser of CUsers) {
    ...
    const foundDouble = CUsers.filter(
      (user) =>
        is_student(user) &&
        incomingUser.customer_phone &&
        user.customer_phone === incomingUser.customer_phone,
    );
