...
const messageTemplates = (templateName, data) => {
  const templates = {
    createUser: `Для Вас создана учетная запись в Личном Кабинете UMED. Там вы можете посмотреть своё расписание, зачетную книжку, оценки и многое другое! Учебная группа: ${data.UF_COLLEDGE_GROUP}. Для входа пройдите по ссылке https://umedcollege.ru/lk Логин: ${data.LOGIN} Пароль: ${data.PASSWORD ? data.PASSWORD : 'СНИЛС без прочерков и пробелов (11 цифр)'}`,
    createParent: `Для Вас создана учетная запись в Личном Кабинете UMED. Там вы можете посмотреть расписание, зачетную книжку, оценки студента и многое другое! Учебная группа: ${data.UF_COLLEDGE_GROUP}. Для входа пройдите по ссылке https://umedcollege.ru/lk Логин: ${data.LOGIN} Пароль: ${data.PASSWORD ? data.PASSWORD : 'СНИЛС студента без прочерков и пробелов (11 цифр)'}`,
    updateUser: `Ваша учетная запись в Личном Кабинете UMED была изменена. Учебная группа: ${data.UF_COLLEDGE_GROUP}. Для входа пройдите по ссылке https://umedcollege.ru/lk Логин: ${data.LOGIN} Пароль: ${data.PASSWORD ? data.PASSWORD : 'СНИЛС без прочерков и пробелов (11 цифр)'}`,
    updateParent: `Ваша учетная запись в Личном Кабинете UMED была изменена. Учебная группа: ${data.UF_COLLEDGE_GROUP}. Для входа пройдите по ссылке https://umedcollege.ru/lk Логин: ${data.LOGIN} Пароль: ${data.PASSWORD ? data.PASSWORD : 'СНИЛС студента без прочерков и пробелов (11 цифр)'}`,
  };
...
async function createStudent(incomingUser = {}, umedUsers) {
  ...
  const phoneExist = umedUsers.find(
    (umedUser) => incomingUser?.LOGIN && umedUser.LOGIN === incomingUser.LOGIN,
  );

  const emailExist = umedUsers.find(
    (umedUser) =>
      incomingUser?.EMAIL &&
      (umedUser.LOGIN === incomingUser.EMAIL || umedUser.EMAIL === incomingUser.EMAIL),
  );
...
async function createParent(incomingUser = {}, umedUsers) {
  ...
  const phoneExist = umedUsers.find(
    (umedUser) =>
      incomingUser?.UF_PARENT_PHONE && umedUser.LOGIN === incomingUser.UF_PARENT_PHONE,
  );
  ...
  const emailExist = umedUsers.find(
    (umedUser) =>
      ...
      (umedUser.LOGIN === incomingUser.UF_PARENT_EMAIL ||
        umedUser.EMAIL === incomingUser.UF_PARENT_EMAIL),
  );
...
async function updateStudent(incomingUser = {}, umedUsers = []) {
  ...
  const phoneExist = umedUsers.find(
    (umedUser) =>
      incomingUser?.LOGIN &&
      umedUser.LOGIN === incomingUser.LOGIN &&
...
const syncUmed = async (CUsers = []) => {
  ...
  const { CStudents_valid, CStudents_invalid } = Object.keys(CUsers).reduce(
    (acc, id) => {
      ...
      if (student) {
        acc.CStudents_valid.push(student);
      } else {
        acc.CStudents_invalid.push(CUsers[id][0]);
      }
