...
const map1CUser = (data = {}) => {
  ...
  if (edu_start) {
    result.UF_PERIOD = utils.convertDate(edu_start);
  }
...
const createTeacher = async (incomingUser = {}, umedUsers) => {
  ...
  const loginPhoneExist = umedUsers.find(
    (el) => el.PHONE_NUMBER === incomingUser.phone || el.LOGIN === incomingUser.phone,
  );
  const emailExist = umedUsers.find((el) => el.EMAIL === incomingUser.email);

  let login = incomingUser.phone;
  let phone = incomingUser.phone;

  if (loginPhoneExist) {
    if (emailExist) {
      return false;
    }
    login = incomingUser.email;
    phone = '';
  }
...
const updateTeacher = async (incomingUser, teacher) => {
  ...
  if (
    forgotToLogin(parseCustomDate(teacher.LAST_LOGIN), 80) &&
    forgotToLogin(parseCustomDate(teacher.TIMESTAMP_X), 85)
  ) {
...
const createStudent = async (incomingUser = {}, umedUsers) => {
  ...
  const paramsData = {
    ...
    UF_PERIOD: incomingUser.UF_PERIOD,
...
const updateStudent = async (incomingUser, umedUsers) => {
  ...
  if (
    forgotToLogin(parseCustomDate(student.LAST_LOGIN), 80) &&
    forgotToLogin(parseCustomDate(student.TIMESTAMP_X), 85)
  ) {
...
const updateParent = async (incomingUser, umedUsers) => {
  ...
  if (
    forgotToLogin(parseCustomDate(parent.LAST_LOGIN), 80) &&
    forgotToLogin(parseCustomDate(parent.TIMESTAMP_X), 85)
  ) {
    paramsData.PASSWORD = incomingUser.UF_SNILS
      ? incomingUser.UF_SNILS
      : incomingUser.UF_APP_PASSWORD;
  ...
  const parent = umedUsers.find(
    (umedUser) =>
      ((umedUser.LOGIN === incomingUser?.UF_PARENT_PHONE &&
        ...
        umedUser.LOGIN === incomingUser?.UF_PARENT_EMAIL ||
        umedUser.LOGIN === '0' + incomingUser?.UF_SNILS) &&
      umedUser.GROUPS_ID.includes(UMED_ABITURIENT_GROUP_ID),
  );
...
const syncUmed = async (CUsers = []) => {
  ...
  // UPGRADE FROM ABITURIENTS TO STUDENTS BY SNILS
  console.log('umed: Upgrade students by phone');
  ...
  const { found: abiturientsToStudentsbySnils } = splitById(
    CStudents_valid,
    umedAbiturients,
    'person_snils',
    'LOGIN',
  ...
  for (const student of abiturientsToStudentsbySnils) {
    await upgradeToStudent(map1CUser(student), umedUsers);
  }
  ////
