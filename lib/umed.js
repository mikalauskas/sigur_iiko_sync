...
function parseCustomDate(dateString) {
  if (utils.isEmpty(dateString)) return false;
...
const messageTemplates = (templateName, data) => {
  const templates = {
    createUser: `Для Вас создана учетная запись в Личном Кабинете UMED. Там вы можете посмотреть своё расписание, зачетную книжку, оценки и многое другое! Для входа пройдите по ссылке https://umedcollege.ru/lk Логин: ${data.LOGIN} Пароль: ${data.PASSWORD ?? 'СНИЛС без прочерков и пробелов (11 цифр)'}`,
    createParent: `Для Вас создана учетная запись в Личном Кабинете UMED. Там вы можете посмотреть расписание, зачетную книжку, оценки студента и многое другое! Для входа пройдите по ссылке https://umedcollege.ru/lk Логин: ${data.LOGIN} Пароль: ${data.PASSWORD ?? 'СНИЛС студента без прочерков и пробелов (11 цифр)'}`,
    updateUser: `Ваша учетная запись в Личном Кабинете UMED была изменена. Для входа пройдите по ссылке https://umedcollege.ru/lk Логин: ${data.LOGIN} Пароль: ${data.PASSWORD ?? 'СНИЛС без прочерков и пробелов (11 цифр)'}`,
    updateParent: `Ваша учетная запись в Личном Кабинете UMED была изменена. Для входа пройдите по ссылке https://umedcollege.ru/lk Логин: ${data.LOGIN} Пароль: ${data.PASSWORD ?? 'СНИЛС студента без прочерков и пробелов (11 цифр)'}`,
  };
...
async function createStudent(incomingUser = {}, umedUsers) {
  ...
  const paramsData = {
    ACTIVE: 'Y',
    LOGIN: incomingUser['LOGIN'],
    PHONE_NUMBER: incomingUser['LOGIN'],
  ...
  const foundUser = umedUsers.find((umedUser) => umedUser.LOGIN === incomingUser?.LOGIN);

  if (foundUser) {
    if (umedUsers.find((umedUser) => umedUser.LOGIN === incomingUser.EMAIL)) {
      paramsData.LOGIN = incomingUser['UF_SNILS'] ?? incomingUser['UF_ID_BITRIX'];
  ...
  console.log(`umed: create user ${paramsData.LOGIN}`, paramsData);

  if (result) {
    console.log(`umed: create user success`);
    ...
    if (incomingUser['PHONE_NUMBER']) {
      wazzup.sendMessage({
        ...
        text: messageTemplates('createUser', {
          ...
          PASSWORD: incomingUser['UF_SNILS'] ?? null,
        }),
      });
    }
...
async function createParent(incomingUser = {}, umedUsers) {
  ...
  if (
    umedUsers.find(
      (umedUser) => umedUser.UF_PARENT_STUDENT_ID === incomingUser.UF_PERSON_ID,
    )
  ) {
    return false;
  }
  ...
  const foundUser = umedUsers.find(
    (umedUser) => umedUser.LOGIN === incomingUser?.UF_PARENT_PHONE,
  );

  if (foundUser) {
    if (umedUsers.find((umedUser) => umedUser.LOGIN === incomingUser.UF_PARENT_EMAIL)) {
      paramsData.LOGIN = '0' + (incomingUser['UF_SNILS'] ?? incomingUser['UF_ID_BITRIX']);

      delete paramsData.PHONE_NUMBER;
    } else if (incomingUser['UF_PARENT_EMAIL']) {
  ...
  console.log(`umed: create parent ${incomingUser.UF_PARENT_PHONE}`, paramsData);
  const result = await postData(paramsData);
...
async function updateStudent(incomingUser = {}, umedUsers = []) {
  ...
  if (
    forgotToLogin(parseCustomDate(student.LAST_LOGIN), 90) &&
    forgotToLogin(parseCustomDate(student.TIMESTAMP_X), 60)
  ) {
  ...
  const doubleStudent = umedUsers.find(
    (umedUser) => umedUser.LOGIN === incomingUser.LOGIN && umedUser.ID !== student.ID,
  );

  if (doubleStudent) {
    if (
      umedUsers.find(
        (umedUser) => umedUser.LOGIN === incomingUser.EMAIL && umedUser.ID !== student.ID,
      )
    ) {
      paramsData.LOGIN = incomingUser['UF_SNILS'] ?? incomingUser['UF_ID_BITRIX'];
      userToUpdate.LOGIN = paramsData.LOGIN;

      delete paramsData.PHONE_NUMBER;
      delete userToUpdate.PHONE_NUMBER;
    } else if (incomingUser['EMAIL']) {
      paramsData.LOGIN = incomingUser['EMAIL'];
  ...
  if (!utils.isEmpty(userToUpdate) && Object.keys(userToUpdate).length) {
    ...
    if (result) {
      console.log(`umed: update user ${student.ID} ${student.LOGIN}`, userToUpdate);
        ...
        if (incomingUser['PHONE_NUMBER']) {
          wazzup.sendMessage({
            ...
            text: messageTemplates('updateUser', {
              LOGIN: paramsData.LOGIN,
              PASSWORD: incomingUser['UF_SNILS'] ? null : paramsData.PASSWORD,
            }),
          });
        }
...
async function updateParent(incomingUser = {}, umedUsers = []) {
  ...
  if (
    forgotToLogin(parseCustomDate(parent.LAST_LOGIN), 90) &&
    forgotToLogin(parseCustomDate(parent.TIMESTAMP_X), 60)
  ) {
    paramsData.PASSWORD = incomingUser['UF_SNILS'] ?? incomingUser['UF_APP_PASSWORD'];
    paramsData.CONFIRM_PASSWORD = paramsData.PASSWORD;
    userToUpdate.PASSWORD = paramsData.PASSWORD;
  ...
  const doubleParent = umedUsers.find(
    (umedUser) =>
      umedUser.LOGIN === incomingUser.UF_PARENT_PHONE && umedUser.ID !== parent.ID,
  );
  ...
  if (doubleParent) {
    if (
      umedUsers.find(
        (umedUser) =>
          umedUser.LOGIN === incomingUser.UF_PARENT_EMAIL && umedUser.ID !== parent.ID,
      )
    ) {
      paramsData.LOGIN = '0' + (incomingUser['UF_SNILS'] ?? incomingUser['UF_ID_BITRIX']);
      userToUpdate.LOGIN = paramsData.LOGIN;
      ...
      incomingUser['UF_PARENT_EMAIL'] &&
      parent.LOGIN !== incomingUser['UF_PARENT_EMAIL']
    ) {
      paramsData.LOGIN = incomingUser['UF_PARENT_EMAIL'];
      userToUpdate.LOGIN = paramsData.LOGIN;
...
async function upgradeToStudent(incomingUser = {}, umedUsers = []) {
  ...
  const student = umedUsers.find(
    (umedUser) =>
      (umedUser.LOGIN === incomingUser.LOGIN || umedUser.LOGIN === incomingUser.EMAIL) &&
...
const syncUmed = async (CUsers = []) => {
  ...
  async function getUmedUsers() {
    return mapUmedUser(await fetchData()).filter((user) => {
      return (
        user.GROUPS_ID.includes(UMED_ABITURIENT_GROUP_ID) ||
  ...
  for (const student of abiturientsToStudentsbyPhone) {
    await upgradeToStudent(map1CUser(student), umedAbiturients);
  }
  ////
  ...
  for (const parent of abiturientsToParentsbyPhone) {
    await upgradeToParent(map1CUser(parent), umedAbiturients);
  }
  ////
  ...
  let umedParents = umedUsers.filter((user) => {
    return (
      user.GROUPS_ID.includes(UMED_PARENT_GROUP_ID) &&
  ...
  await utils.writeToJson('studentsToUpdate.json', studentsToUpdate);
