...
const findTeacher = (teacher, umedUsers) => {
  return umedUsers.find((umedUser) => umedUser.UF_TEACHER === teacher.fullname);
};
...
const messageTemplates = (templateName, data) => {
  const templates = {
    createTeacher: `Привет, Преподаватель! 
    Для Вас создана учетная запись в Личном Кабинете UMED. 
    Там вы можете посмотреть своё расписание, зайти в эл. библиотеку и многое другое!  
    ...
    Ваша учетная запись в Личном Кабинете UMED была изменена. 
    Учебная группа: ${data.UF_COLLEDGE_GROUP}. 
    Для входа пройдите по ссылке https://umedcollege.ru/lk 
    Логин: ${data.LOGIN} 
    Пароль: ${data.PASSWORD ? data.PASSWORD : 'СНИЛС без прочерков и пробелов (11 цифр)'}`,
...
async function createTeacher(incomingUser = {}, umedUsers) {
...
async function getUmedUsersTeachers() {
  return mapUmedTeacher(await fetchData()).filter((user) => {
    return (
      user?.GROUPS_ID?.includes(UMED_ABITURIENT_GROUP_ID) ||
      user?.GROUPS_ID?.includes(UMED_STUDENT_GROUP_ID) ||
      user?.GROUPS_ID?.includes(UMED_PARENT_GROUP_ID) ||
...
module.exports = { syncUmed, findTeacher, getUmedUsersTeachers, createTeacher };
