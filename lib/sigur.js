const Database = require('./db.js');
const dotenv = require('dotenv');
dotenv.config();

const sigurDbHost = process.env.SIGUR_HOST;
const SigurDbPort = process.env.SIGUR_PORT;
const SigurDbUser = process.env.SIGUR_USER;
const SigurDbPassword = process.env.SIGUR_PASSWORD;
const SigurDbName = process.env.SIGUR_DATABASE;

module.exports = class Sigur extends Database {
  constructor() {
    super(sigurDbHost, SigurDbPort, SigurDbUser, SigurDbPassword, SigurDbName);
  }

  async addGroup(groupName) {
    if (!groupName) return false;
    return new Promise((resolve, reject) => {
      const query = `
      INSERT
        INTO
        \`tc-db-main\`.personal (PARENT_ID,
        \`TYPE\`,
        EMP_TYPE,
        NAME,
        STATUS)
      VALUES (1653,
      'DEP',
      'EMP',
      '${groupName}',
      'AVAILABLE');
      `;

      const options = { sql: query };

      this.db.query(options, (error, results) => {
        console.log(`Sending data to ${this.database}`);
        if (error) reject(error);

        resolve(results);
      });
    });
  }

  /**
   * add user to sigur
   *
   * @async
   * @param {int} groupId
   * @param {String} fullname
   * @param {String} position
   * @param {String} person_id
   * @returns {void}
   */
  async addPersonal(groupId, fullname, position, person_id) {
    // Log the incoming data for the new user
    console.log(
      `Sigur: Received request to add new user with details - Group ID: ${groupId}, Full Name: "${fullname}", Position: "${position}", Person ID: "${person_id}"`,
    );

    // Validate the essential parameters before proceeding
    if (!groupId || !fullname || !position) {
      console.log(
        `Sigur: Missing required information (Group ID: ${groupId}, Full Name: "${fullname}", Position: "${position}"). Insert aborted.`,
      );
      return false;
    }
    return new Promise((resolve, reject) => {
      const query = `
      INSERT
        INTO
        \`tc-db-main\`.personal (PARENT_ID,
        \`TYPE\`,
        EMP_TYPE,
        NAME,
        POS,
        STATUS,
        TABID)
      VALUES (${groupId},
      'EMP',
      'EMP',
      '${fullname}',
      '${position}',
      'AVAILABLE',
      '${person_id}');
      `;

      const options = { sql: query };

      // Log the database interaction attempt
      console.log(`Sigur: Sending data to the database: ${this.database}`);

      // Execute the query on the database
      this.db.query(options, (error, results) => {
        if (error) {
          // Log the error if the query fails
          console.error(
            `Sigur: Error inserting new user into database - ${error.message}`,
          );
          reject(error);
        } else {
          // Log the successful insertion
          console.log(
            `Sigur: Successfully added new user "${fullname}" to group ${groupId}.`,
          );
          resolve(results);
        }
      });
    });
  }

  /**
   * update user in sigur
   *
   * @async
   * @param {int} userId
   * @param {int} groupId
   * @param {String} fullname
   * @param {String} position
   * @param {String} person_id
   * @returns {void}
   */
  async updatePersonal(userId, groupId, fullname, position, person_id) {
    userId = Number(userId);
    groupId = Number(groupId);

    // Log the incoming data before processing
    console.log(
      `sigur: Received request to update user with details - User ID: ${userId}, Group ID: ${groupId}, Full Name: "${fullname}", Position: "${position}", Person ID: "${person_id}"`,
    );

    // Validate the userId and groupId before proceeding
    if (!userId || !groupId) {
      console.log(
        `sigur: Invalid User ID (${userId}) or Group ID (${groupId}). Update aborted.`,
      );
      return false;
    }

    return new Promise((resolve, reject) => {
      const query = `
      UPDATE \`tc-db-main\`.personal
      SET NAME = '${fullname}',
          PARENT_ID = ${groupId},
          POS = '${position}',
          TABID = '${person_id}'
      WHERE ID = ${userId};
      `;

      const options = { sql: query };

      this.db.query(options, (error, results) => {
        console.log(`Sending data to ${this.database}`);
        if (error) {
          // Log the error if the query fails
          console.error(`sigur: Error updating user in database - ${error.message}`);
          reject(error);
        } else {
          // Log the successful update
          console.log(
            `sigur: Successfully updated user ${userId} in group ${groupId} with new details.`,
          );
          resolve(results);
        }
      });
    });
  }

  /**
   * get all students
   *
   * @async
   * @returns {[ID: Number, NAME: String, POS: String, CODEKEY: String]}
   */
  async getPersonal() {
    return new Promise((resolve, reject) => {
      const query = `
        SELECT
            t1.ID AS ID,
            t1.PARENT_ID AS PARENT_ID,
            t1.NAME AS NAME,
            t1.POS AS POS,
            t1.CODEKEY AS CODEKEY,
            t1.DESCRIPTION AS DESCRIPTION,
            t1.TABID AS TABID
        FROM
            \`tc-db-main\`.personal AS t1
        WHERE
            t1.NAME IS NOT NULL
            AND EXISTS (
                SELECT 1
                FROM
                    \`tc-db-main\`.personal AS subquery
                WHERE
                    subquery.TYPE = 'DEP'
                    AND subquery.PARENT_ID = 1653
                    AND subquery.ID = t1.PARENT_ID
            )
        GROUP BY
            ID, NAME
      `;

      const options = { sql: query };

      this.db.query(options, (error, results) => {
        if (error) {
          // Log the error if the query fails
          console.error(`sigur: Error get personal in database - ${error.message}`);
          reject(error);
        } else {
          // Log the successful update
          console.log(`sigur: Successfully got personal.`);
          resolve(results);
        }
      });
    });
  }

  /**
   * get groups
   *
   * @async
   * @returns {[{ID: Number, NAME: String}]}
   */
  async getGroups() {
    return new Promise((resolve, reject) => {
      const query = `
        SELECT
          t1.ID AS ID,
          t1.NAME AS NAME
        FROM
          \`tc-db-main\`.personal AS t1
        WHERE
          t1.NAME IS NOT NULL
          AND t1.\`TYPE\` = 'DEP'
          AND t1.PARENT_ID = 1653;
      `;

      const options = { sql: query };

      this.db.query(options, (error, results) => {
        if (error) {
          // Log the error if the query fails
          console.error(`sigur: Error get groups in database - ${error.message}`);
          reject(error);
        } else {
          // Log the successful update
          console.log(`sigur: Successfully got groups.`);
          resolve(results);
        }
      });
    });
  }

  finish() {
    this.closeConnection();
  }
};
