...
module.exports = class Sigur extends Database {
    ...
    getPersonal(position, callback) {
        ...
        */
        const options = {
            sql: 'SELECT t1.ID AS ID, t1.NAME AS NAME, t1.POS AS POS, t1.CODEKEY AS CODEKEY FROM `tc-db-main`.personal AS t1 JOIN `tc-db-log`.`logs` AS t2 ON t1.ID = t2.EMPHINT WHERE t1.POS LIKE "' + position + '" AND t1.NAME IS NOT NULL AND t1.CODEKEY IS NOT NULL AND t2.LOGTIME >= DATE_SUB(NOW(), INTERVAL 60 DAY) GROUP BY ID, NAME'
        };
