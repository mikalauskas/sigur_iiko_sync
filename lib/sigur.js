...
module.exports = class Sigur extends Database {
	...
	getPersonal(callback) {
		const options = {
			sql: 'SELECT t1.ID AS ID, t1.NAME AS NAME, t1.POS AS POS, t1.CODEKEY AS CODEKEY FROM `tc-db-main`.personal AS t1 WHERE t1.POS LIKE "%туден%" AND t1.NAME IS NOT NULL AND t1.CODEKEY IS NOT NULL'
		};
		this.db.query(options, (error, results, fields) => {
