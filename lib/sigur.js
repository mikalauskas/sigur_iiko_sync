...
module.exports = class Sigur extends Database {
  ...
  async disablePersonal(userId, position) {
    ...
    return new Promise((resolve, reject) => {
      const query = `
      UPDATE \`tc-db-main\`.personal
      SET POS = ?, EXPTIME = "1900-01-01 23:59:00.000"
      WHERE ID = ?;
      `;
  ...
  async deletePersonal(userId) {
    ...
    return new Promise((resolve, reject) => {
      const query = `
      UPDATE \`tc-db-main\`.personal
      SET STATUS = "FIRED", EXPTIME = "1900-01-01 23:59:00.000"
      WHERE ID = ?;
      `;
  ...
  async cleanTable() {
    const query = `UPDATE \`tc-db-main\`.personal
                    SET STATUS = 'FIRED', EXPTIME = "1900-01-01 23:59:00.000"
                    WHERE (personal.CODEKEY IS NULL OR personal.LOCATIONACT IS NULL 
                        OR personal.LOCATIONACT < DATE_SUB(?, INTERVAL 180 DAY))
                        AND personal.CREATEDTIME < DATE_SUB(?, INTERVAL 14 DAY)
                    AND TYPE = 'EMP'
                    AND STATUS = 'AVAILABLE'
                    AND ID > 1000 
                    AND ((POS = 'ВАкадемическомОтпуске' 
                        OR POS = 'Отчислен' 
                        OR POS = 'Выпущен' 
                        OR POS = 'Студент') OR (LENGTH(CAST(TABID AS CHAR)) = 11 
                    AND TABID REGEXP '^[0-9]{11}$'));`;
