...
const explodeFullName = (fullName) => {
  ...
  const nameParts = fullName
    .split(/\s+/u)
    .map((part) => part.replace(/[^a-zA-Zа-яёА-ЯЁ-]/g, ''))
    .filter(Boolean)
    .map((word) => {
      return word[0].toUpperCase() + word.substring(1);
    });

  const [firstName = '', middleName = '', lastName = '', ...otherParts] = nameParts;
  const otherNames = otherParts
    .map((part) => part.replace(/\(-\)/, '').replace(/[^a-zA-Zа-яёА-ЯЁ-]/g, ''))
    .filter(Boolean)
    .join(' ');

  return { firstName, middleName, lastName, otherNames };
};
