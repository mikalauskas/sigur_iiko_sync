...
const jobtitleModel = (data = []) => {
  ...
  return data
    .map((el) => {
      const record = el.RecordSet[0];
      const employee_id = record.Сотрудник_Key;
      const jobTitle = record.Должность_Key;
      const jobTitle2 = record.ДолжностьПоШтатномуРасписанию_Key;
      const jobStatus = record.ВидСобытия;
    ...
    .reduce((acc, curr) => {
      if (curr.employee_id !== undefined && curr.jobStatus !== undefined) {
        const found = acc.find((item) => item.employee_id === curr.employee_id);
        if (found) {
          if (curr.jobStatus) {
            found.jobStatus.push(...curr.jobStatus);
          }
        } else {
          acc.push({
            employee_id: curr.employee_id,
            jobTitle: curr.jobTitle,
            jobTitle2: curr.jobTitle2,
            jobStatus: curr.jobStatus,
          });
        }
      }
      return acc;
...
const create1cFreshJsonData = async (endpoint = '', login = '', password = '') => {
  ...
  try {
    ...
    const jobStatusList = await fetchData(
      'InformationRegister_КадроваяИсторияСотрудников',
      '1cFresh_jobstatus.json',
      jobtitleModel,
    );
    ...
    if (!isArrayEmpty(employeeList && personList && statusList)) {
      ...
      const merge4 = mergeArrays(merge3, jobStatusList, 'employee_id');
      const finalValidData = filterValid(merge4);
