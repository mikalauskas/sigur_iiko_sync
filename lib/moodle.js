...
const createUser = async (incomingUser = {}, moodleUsers = []) => {
  ...
  if (doubleStudent) {
    ...
    incomingUser.login2 = incomingUser.phone;
    createUser['users[0][username]'] = incomingUser.login2;
  }
...
const updateUser = async (user = {}, moodleUsers = [], incomingUser = {}) => {
  ...
  if (user.username !== incomingUser.login2) {
    postData['users[0][username]'] = incomingUser.login2;
    needsUpdate = true;
  }

  if (!incomingUser.email && user.email !== incomingUser.login2 + '@mail.ru') {
    postData['users[0][email]'] = incomingUser.login2 + '@mail.ru';
    needsUpdate = true;
  }
