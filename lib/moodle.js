...
const addUserToCohort = async (user = {}, incomingUser = {}, cohort = {}) => {
  if (utils.isEmpty(user || incomingUser || cohort?.id)) return false;
  ...
  for (const incomingUser of CUsers) {
    ...
    if (is_student(incomingUser) && farm2023(incomingUser)) {
      if (user && user.id) {
        ...
        user = createUser(incomingUser, moodleUsers);
      }
      ...
      if (utils.isEmpty(cohort)) cohort = await createCohort(incomingUser);

      const userids = await getCohortsMembers(cohort);

      if (!userids.includes(user.id)) await addUserToCohort(user, incomingUser, cohort);
    } else {
