...
const handleGlobalCohortAdd = async (userId, incomingUser = {}) => {
  ...
  addCohortMembersData[`members[0][cohorttype][value]`] =
    GLOBAL_GROUPS[remapForms[incomingUser.edu_form]];
  ...
  addCohortMembersData[`members[1][cohorttype][value]`] =
    GLOBAL_GROUPS[remapForms[incomingUser.speciality_name]];
  ...
  const combinedForm =
    (remapForms[incomingUser.speciality_name]
      ? remapForms[incomingUser.speciality_name]
      : '') +
    (remapForms[incomingUser.edu_form] ? remapForms[incomingUser.edu_form] : '');

  addCohortMembersData[`members[2][cohorttype][type]`] = 'id';
  addCohortMembersData[`members[2][cohorttype][value]`] = GLOBAL_GROUPS[combinedForm];
  addCohortMembersData[`members[2][usertype][type]`] = 'id';
  addCohortMembersData[`members[2][usertype][value]`] = userId;

  addCohortMembersData['members[3][cohorttype][type]'] = 'id';
  addCohortMembersData['members[3][cohorttype][value]'] = GLOBAL_GROUPS.STUDENTS;
  addCohortMembersData['members[3][usertype][type]'] = 'id';
  addCohortMembersData['members[3][usertype][value]'] = userId;
...
const syncMoodle = async (CUsers = []) => {
  ...
  try {
    ...
    for (const incomingUser of CStudents_valid) {
      ...
      console.log('Starting handleRole job');
      await handleRole(student.id, STUDENT_MOODLE_ROLE_ID);
      console.log('Finished handleRole job');
      console.log('Starting handleCohort job');
      await handleCohortAdd(incomingUser, student);
      console.log('Finished handleCohort job');
      console.log('Starting handleGlobalCohort job');
      await handleGlobalCohortAdd(student.id, incomingUser);
      console.log('Finished handleGlobalCohort job');
      console.log('Starting handleCourses job');
      await handleCourses(student);
      console.log('Finished handleCourses job');
      console.log('Starting handleCohortRemove job');
