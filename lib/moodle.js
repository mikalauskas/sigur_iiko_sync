...
const getUsersCohorts = async (moodleUser, umedUser, cohortIds) => {
  let userCohortIds = [];
...
const handleCohort = async (incomingUser, moodleUsers) => {
  let student = findUser(incomingUser, moodleUsers);
  ...
  await addUserToGlobalCohort(student, incomingUser);
};

const handleCohort2 = async (incomingUser, moodleUsers) => {
  ...
  await getUsersCohorts(student, incomingUser, cohortIds);
};
...
const syncMoodle = async (CUsers = []) => {
  ...
  try {
    ...
    for (const incomingUser of CStudents_valid) {
      await handleCohort(incomingUser, moodleUsers);
    }
    ...
    for (const incomingUser of CStudents_valid) {
      await handleCohort2(incomingUser, moodleUsers);
    }
