...
const updateUser = async (user = {}, moodleUsers = [], incomingUser = {}) => {
  ...
  // Check if a user with matching phone number already exists in Moodle
  const phoneExist = moodleUsers.find(
    (moodleUser) =>
      moodleUser.username === incomingUser.phone && moodleUser.id !== user.id,
  );

  // Check if a user with matching email address already exists in Moodle
  ...
  // If a user with matching phone number exists, but not a user with matching email address:
  //   - If the user has an email address, set login2 to email.
  //   - Otherwise, leave login2 unset (i.e. set it to whatever the user originally had).
  if (phoneExist) {
    if (emailExist) {
      incomingUser.login2 = incomingUser.person_snils || incomingUser.login;
    } else if (incomingUser.email) {
    ...
    incomingUser.login2 =
      incomingUser.phone || incomingUser.email || incomingUser.person_snils;
  }
