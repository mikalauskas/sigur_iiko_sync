...
const handleStudentUser = async (incomingUser, user, moodleUsers) => {
  if (user?.id) {
    await updateUser(user, moodleUsers, map1CUser(incomingUser));
  } else {
    user = await createUser(map1CUser(incomingUser), moodleUsers);
  }
  ...
  const userCourses = await getUserCourses(user);

  await updateGroupMembership(user, userCourses);

  return user;
};

const handleNonStudentUser = async (user) => {
  if (user?.id && !user.suspended) {
    const suspended = await suspendUser(user.id);
    if (suspended) console.log(`moodle: suspended user ${user.student_id}`);
  }
...
const syncMoodle = async (cUsers = []) => {
  ...
  try {
    ...
    for (const incomingUser of cUsers) {
      ...
      if (isStudent(incomingUser)) {
        user = await handleStudentUser(incomingUser, user, moodleUsers);
      } else {
        await handleNonStudentUser(user);
      }
    }
