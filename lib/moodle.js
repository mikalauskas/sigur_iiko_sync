 ...
 * @returns {Promise<any>} - A promise that resolves when the user is added to the cohort or rejects if there's an error.
 */
const addUserToGlobalCohort = async (user = {}, incomingUser = {}) => {
  if (utils.isEmpty(user?.id) || utils.isEmpty(incomingUser)) return false;
  ...
  switch (incomingUser.edu_form) {
    case 'Очно-заочная':
      ...
      addCohortMembersData['members[0][usertype][value]'] = user.id;
      break;
    case 'Заочная':
      addCohortMembersData['members[0][cohorttype][type]'] = 'id';
  ...
  try {
    ...
    if (!utils.isEmpty(addCohortMembersResult?.warnings)) {
      ...
      console.log('moodle: User added to global cohorts successfully.');
    }
  } catch (error) {
 ...
 * @returns {Promise<any>} - A promise that resolves to an array of user courses or rejects if there's an error.
 */
...
const getCreditBook = async (StudentID) => {
  ...
  try {
    ...
    console.error('Error fetching credit book:', error);
    return [];
  }
};
 ...
 * @returns {Promise<any>} - A promise that resolves when the group membership is updated or rejects if there's an error.
 */
 ...
 *
 * @param {Object} student - User data.
 * @param {number} student.id - User's Moodle ID.
...
const RemoveUserFromGlobalCohort = async (moodleUser, umedUser, cohortIds) => {
  let userCohortIds = [];

  function removeIdsFromArray(id) {
    const index = userCohortIds.indexOf(id);
  ...
  for (const cohort of cohortIds) {
    ...
    if (Array.isArray(userIds) && userIds.includes(moodleUser.id)) {
      userCohortIds.push(cohort.id);
    }
  }

  const groupCohort = await searchCohort(umedUser);

  removeIdsFromArray(groupCohort?.id);
  removeIdsFromArray(STUDENTS_MOODLE_GROUP_ID);

  switch (umedUser.edu_form) {
    case 'Очно-заочная':
  ...
  switch (umedUser.speciality_name) {
    case 'Фармация':
  ...
  if (Array.isArray(userCohortIds) && userCohortIds.length > 0) {
    for (const id of userCohortIds) {
      const cohortData = {
        wstoken: token,
        wsfunction: 'core_cohort_delete_cohort_members',
        moodlewsrestformat: 'json',
        'members[0][userid]': moodleUser.id,
        'members[0][cohortid]': id,
      };

      console.log(`Removing user ${moodleUser.id} from cohort ${id}`);
      await moodleCurl(cohortData);
    }
  }
};

const updateCreateStudent = async (incomingUser, moodleUsers) => {
  let student = findUser(incomingUser, moodleUsers);

  if (student?.id) {
    await updateUser(student, moodleUsers, map1CUser(incomingUser));
  } else {
    student = await createUser(map1CUser(incomingUser), moodleUsers);
  }
};

const setRoleStudent = async (incomingUser, moodleUsers) => {
  let student = findUser(incomingUser, moodleUsers);
...
const handleCohortAdd = async (incomingUser, moodleUsers) => {
  ...
  if (Array.isArray(userIds) && !userIds.includes(student.id)) {
    await addUserToCohort(student, incomingUser, cohort);
  }

  addUserToGlobalCohort(student, incomingUser);
};

const handleCohortRemove = async (incomingUser, moodleUsers, cohortIds) => {
  const student = findUser(incomingUser, moodleUsers);

  if (!student?.id) return false;

  RemoveUserFromGlobalCohort(student, incomingUser, cohortIds);
};

const handleCourses = async (incomingUser, moodleUsers) => {
  const student = findUser(incomingUser, moodleUsers);

  if (!student?.id) return false;

  const userCourses = await getUserCourses(student);

  await updateGroupMembership(student, userCourses);
};

const handleNonStudentUser = async (incomingUser, moodleUsers) => {
  const student = findUser(incomingUser, moodleUsers);

  if (student?.id && !student.suspended) {
    const suspended = await suspendUser(student.id);
    if (suspended) console.log(`moodle: suspended user ${student.student_id}`);
  }

  if (student?.id) {
    await removeUserFromCohorts(student.id);
  }
};
...
const syncMoodle = async (CUsers = []) => {
  ...
  try {
    ...
    console.log('moodle: updateCreateStudent job started');

    moodleUsers = await getUsers();

    for (const incomingUser of CStudents_valid) {
      await updateCreateStudent(incomingUser, moodleUsers);
    }

    console.log('moodle: setRoleStudent job started');

    moodleUsers = await getUsers();

    for (const incomingUser of CStudents_valid) {
      await setRoleStudent(incomingUser, moodleUsers);
    }

    console.log('moodle: handleCohort job started');

    moodleUsers = await getUsers();

    for (const incomingUser of CStudents_valid) {
      await handleCohortAdd(incomingUser, moodleUsers);
    }

    console.log('moodle: handleCourses job started');

    moodleUsers = await getUsers();

    for (const incomingUser of CStudents_valid) {
      await handleCourses(incomingUser, moodleUsers);
    }

    console.log('moodle: handleNonStudentUser job started');

    moodleUsers = await getUsers();

    for (const incomingUser of CStudents_invalid) {
      await handleNonStudentUser(incomingUser, moodleUsers);
    }

    console.log('moodle: handleCohort2 job started');
    ...
    for (const incomingUser of CStudents_valid) {
      await handleCohortRemove(incomingUser, moodleUsers, cohortIds);
    }
    ...
    console.error('Error during syncMoodle job:', error);
    return false;
  }
};
