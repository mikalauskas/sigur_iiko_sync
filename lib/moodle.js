...
const createUser = async (incomingUser = {}, moodleUsers = []) => {
  ...
  const paramsData = {
    ...
    'users[0][email]': incomingUser.email
      ? incomingUser.email
      : (incomingUser.phone || incomingUser.person_snils || incomingUser.login) +
        '@mail.ru',
  ...
  if (phoneExist) {
    if (emailExist || !incomingUser?.email) {
      paramsData['users[0][username]'] = incomingUser.person_snils || incomingUser.login;
    } else {
...
const updateUser = async (user = {}, moodleUsers = [], incomingUser = {}) => {
  ...
  const snilsExist = moodleUsers.find(
    (moodleUser) =>
      incomingUser?.person_snils &&
      moodleUser.username === incomingUser.person_snils &&
      moodleUser.id !== user.id,
  );

  if (phoneExist) {
    if (snilsExist) {
      incomingUser.login2 = incomingUser.login;
    } else if (emailExist && incomingUser?.person_snils) {
      incomingUser.login2 = incomingUser.person_snils;
  ...
  if (incomingUser.login2 && user.username !== incomingUser.login2) {
    postData['users[0][username]'] = incomingUser.login2;
    needsUpdate = true;
  }

  incomingUser.email2 = incomingUser.login2 + '@mail.ru';

  if (!incomingUser.email && user.email !== incomingUser.email2) {
    postData['users[0][email]'] = incomingUser.email2;
    needsUpdate = true;
  }
