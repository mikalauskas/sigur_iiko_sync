...
const getUsersCohorts = async (user) => {
  const cohortsData = {
    wstoken: token,
    wsfunction: 'core_cohort_get_cohorts',
    moodlewsrestformat: 'json',
  ...
  for (const id of cohortIds) {
    const userIds = await getCohortsMembers({ id: id });

    if (Array.isArray(userIds) && userIds.includes(user.id)) {
      userCohortIds.push(id);
    }
  }

  const groupCohort = await searchCohort(user);

  IdToRemove.push(groupCohort?.id);

  switch (user.edu_form) {
    case 'Очно-заочная':
      IdToRemove.push(USK_MOODLE_GROUP_ID);
      break;
    case 'Заочная':
      IdToRemove.push(ZAOCH_MOODLE_GROUP_ID);
      break;
    case 'Очная':
      IdToRemove.push(SPO_MOODLE_GROUP_ID);
      break;
  }

  switch (user.speciality_name) {
    case 'Фармация':
      IdToRemove.push(FARMA_MOODLE_GROUP_ID);
      break;
    case 'Лечебное дело':
      IdToRemove.push(LECH_MOODLE_GROUP_ID);
      break;
    case 'Сестринское дело':
      IdToRemove.push(SESTR_MOODLE_GROUP_ID);
      break;
    case 'Медицинская оптика':
      IdToRemove.push(OPTIC_MOODLE_GROUP_ID);
      break;
    case 'Стоматология профилактическая':
      IdToRemove.push(STOM_MOODLE_GROUP_ID);
      break;
  }
};
