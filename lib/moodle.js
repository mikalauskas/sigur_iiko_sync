...
const handleNonStudentUser = async (student) => {
...
const syncMoodle = async (CUsers = []) => {
  ...
  try {
    ...
    for (const incomingUser of CStudents_invalid) {
      ...
      handleNonStudentUser(student);
    }

    console.log(
      'moodle: setRoleStudent handleCohortAdd handleCourses handleCohortRemove job started',
    );
