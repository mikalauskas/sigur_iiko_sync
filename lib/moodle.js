...
const createUser = async (incomingUser = {}, moodleUsers = []) => {
  ...
  if (foundUser) {
    console.log(
      `moodle: found double user ${incomingUser.student_id} with phone ${incomingUser.phone}`,
    );
    if (moodleUsers.find((moodleUser) => moodleUser.username === incomingUser.email)) {
      createUser['users[0][username]'] = incomingUser.person_snils ?? incomingUser.login;
    }

    if (incomingUser.email) createUser['users[0][username]'] = incomingUser.email;
...
const updateUser = async (user = {}, moodleUsers = [], incomingUser = {}) => {
  ...
  const paramsData = {
    ...
    'users[0][email]': incomingUser.email,
    'users[0][phone1]': incomingUser.phone,
    'users[0][department]': incomingUser.department,
    'users[0][idnumber]': incomingUser.idnumber,
    'users[0][description]': incomingUser.description,
    'users[0][person_id]': incomingUser.person_id,
    'users[0][student_id]': incomingUser.student_id,
    'users[0][person_snils]': incomingUser.person_snils,
    'users[0][college_group]': incomingUser.college_group,
    'users[0][info_login]': incomingUser.info_login,
    'users[0][info_pass]': incomingUser.info_pass,
  };
  ...
  const keyMapping = {
    ...
    'users[0][middlename]': 'middlename',
    'users[0][email]': 'email',
    'users[0][phone1]': 'phone1',
    'users[0][department]': 'department',
    'users[0][idnumber]': 'idnumber',
    'users[0][description]': 'description',
    'users[0][person_id]': 'person_id',
    'users[0][student_id]': 'student_id',
    'users[0][person_snils]': 'person_snils',
    'users[0][college_group]': 'college_group',
    'users[0][info_login]': 'info_login',
    'users[0][info_pass]': 'info_pass',
  };
  ...
  for (const key in paramsData) {
    const secondKey = keyMapping[key];
    if (secondKey && user[secondKey] !== undefined) {
      //console.log(`Comparing ${key} with ${secondKey}`);
      if (paramsData[key] !== user[secondKey] && paramsData[key] != undefined) {
        postData[key] = paramsData[key];
      }
    }
  }
  ...
  if (doubleStudent) {
    if (
      ...
      postData['users[0][username]'] = paramsData['users[0][username]'];
    } else if (incomingUser.email) {
      paramsData['users[0][username]'] = incomingUser.email;
      postData['users[0][username]'] = paramsData['users[0][username]'];
    }
  } else {
    paramsData['users[0][username]'] = incomingUser.phone;
    postData['users[0][username]'] = paramsData['users[0][username]'];
  }

  if (!incomingUser.email) {
    paramsData['users[0][email]'] = paramsData['users[0][username]'] + '@mail.ru';
    postData['users[0][email]'] = paramsData['users[0][email]'];
  }

  let customFieldCount = 0;

  if (user.college_group !== incomingUser.group_name) {
    paramsData['users[0][customfields][' + customFieldCount + '][type]'] =
      'college_group';
    paramsData['users[0][customfields][' + customFieldCount + '][value]'] =
      incomingUser.group_name;
    postData['users[0][customfields][' + customFieldCount + '][type]'] =
      paramsData['users[0][customfields][' + customFieldCount + '][type]'];
  ...
  if (user.info_login !== incomingUser.bitrix_id) {
    paramsData['users[0][customfields][' + customFieldCount + '][type]'] = 'info_login';
    paramsData['users[0][customfields][' + customFieldCount + '][value]'] =
  ...
  if (user.info_pass !== incomingUser.password) {
    paramsData['users[0][password]'] = incomingUser.password;
    paramsData['users[0][customfields][' + customFieldCount + '][type]'] = 'info_pass';
    paramsData['users[0][customfields][' + customFieldCount + '][value]'] =
      incomingUser.password;
    customFieldCount++;
  }
  if (user.person_id !== incomingUser.person_id) {
    paramsData['users[0][customfields][' + customFieldCount + '][type]'] = 'person_id';
    paramsData['users[0][customfields][' + customFieldCount + '][value]'] =
      incomingUser.person_id;
    customFieldCount++;
  }
  if (user.student_id !== incomingUser.student_id) {
    paramsData['users[0][customfields][' + customFieldCount + '][type]'] = 'student_id';
    paramsData['users[0][customfields][' + customFieldCount + '][value]'] =
      incomingUser.student_id;
    customFieldCount++;
  }
  if (user.person_snils !== incomingUser.person_snils) {
    paramsData['users[0][customfields][' + customFieldCount + '][type]'] = 'person_snils';
    paramsData['users[0][customfields][' + customFieldCount + '][value]'] =
      incomingUser.person_snils;
    customFieldCount++;
  }
  if (user.suspended !== false) {
    paramsData['users[0][suspended]'] = 0;
  }
  if (user.confirmed !== true) {
    paramsData['users[0][confirmed]'] = 1;
  }
...
const syncMoodle = async (CUsers = []) => {
  ...
  for (const incomingUser of CUsers) {
    ...
    if (
      is_student(
        incomingUser,
      ) /* && (farm2023(incomingUser) || sestr2024(incomingUser)) */
    ) {
      if (user?.id) {
