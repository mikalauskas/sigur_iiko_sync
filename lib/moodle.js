 ...
 * @returns {Array} - array of search results or an empty array.
 */
 ...
 * @returns {Object|boolean} The course object if found, otherwise false.
 */
 ...
 * @returns {Array} An array of course IDs that the user should be enrolled in.
 */
...
const getUsersCohorts = async (moodleUser, umedUser) => {
  const cohortsData = {
    wstoken: token,
    wsfunction: 'core_cohort_get_cohorts',
    moodlewsrestformat: 'json',
...
const handleCohort2 = async (incomingUser, moodleUsers) => {
  ...
  await getUsersCohorts(student, incomingUser);
};
...
module.exports = { syncMoodle, getUsers, findTeacher, createTeacher };
