...
const create1cJsonData = async () => {
  ...
  try {
    ...
    const contractList = await fetchData(
      'Document_КонтрактНаОбучение?$select=Ref_Key,Абитуриент_Key,Специальность_Key,ПрограммаОбучения_Key,Number,Date,ДатаОкончанияКонтракта,ЗаказчикОнЖе,Заказчик,ТелефонЗаказчика,ЭлектроннаяПочтаЗаказчика,ЭлектроннаяПочтаЗаказчика,ЗаказчикЮридическоеЛицо,СтоимостьОбучения,умкГрафикОплаты',
      '1c_contracts.json',
      contractModel,
    );
