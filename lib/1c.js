const { groupArrayByKey, mergeArrays } = require('./utils.js');
const convert = require('xml-js');
const fs = require('node:fs/promises');
const parsePhoneNumber = require('libphonenumber-js/max');
const parseEmail = require('email-addresses');

const TWO_DAYS_IN_MILLISECONDS = 24 * 60 * 60 * 1000; // 2 days in milliseconds
...
const sendOdata = async (catalog, id, data) => {
  ...
  const xmlProperties = Object.keys(data).map((key) => `<d:${key}>${data[key]}</d:${key}>`);
...
const groupModel = (data) => {
  return data
    .map((el) => {
      ...
      if (properties['d:IsFolder']._text === 'true' || properties['d:Статус']._text !== 'Учится')
        return null;
    ...
    .map((el) => {
      ...
      if (el.phone && Object.hasOwn(el, 'phone')) {
        ...
        if (phone && phone.getType() === 'MOBILE') {
          el.phone = '7' + phone.nationalNumber;
        } else {
          delete el.phone;
        }
      }
      ...
      if (el.email && Object.hasOwn(el, 'email')) {
        ...
        if (Object.hasOwn(parseEmail, 'address')) {
          el.email = parsedEmail.address;
        } else {
          delete el.email;
        }
      }
      ...
      if (el.customer_phone && Object.hasOwn(el, 'customer_phone')) {
        ...
        if (customer_phone && customer_phone.getType() === 'MOBILE') {
          el.customer_phone = '7' + customer_phone.nationalNumber;
        } else {
      ...
      if (el.customer_email && Object.hasOwn(el, 'customer_email')) {
        ...
        if (Object.hasOwn(parseEmail, 'address')) {
          el.customer_email = parsedEmail.address;
        } else {
          delete el.customer_email;
        }
      }

      if (Object.hasOwn(el, 'customer_name')) {
        el.customer_name = el.customer_name.trim();
      }

      if (Object.hasOwn(el, 'login')) {
        el.login = el.login.trim();
      }

      if (Object.hasOwn(el, 'password')) {
...
const create1cJsonData = async () => {
  try {
    const studentList = await fetchData('Catalog_Студенты', '1c_students.json', studentModel);
    ...
    const groupList = await fetchData('Catalog_УчебныеГруппы', '1c_groups.json', groupModel);
    ...
    const mergedContact = mergeArrays(studentList, contactInfoList, 'person_id');
    const mergedSpeciality = mergeArrays(mergedContact, specialityList, 'speciality_id');
    const mergedStatus = mergeArrays(mergedSpeciality, statusList, 'student_id');
