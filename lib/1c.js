...
const personModel = (data = []) => {
  ...
  return data.map((el) => {
    ...
    if (!isFolder) {
      const contactInfo = el.КонтактнаяИнформация;

      if (Array.isArray(contactInfo)) {
        const phoneList = contactInfo.filter((el) => el.Тип === 'Телефон');
        const emailList = contactInfo.filter((el) => el.Тип === 'АдресЭлектроннойПочты');

        if (Array.isArray(phoneList)) {
          phone = phoneList[phoneList.length - 1]?.НомерТелефона;
        } else {
          phone = phoneList?.НомерТелефона;
        }

        if (Array.isArray(emailList)) {
          email = emailList[emailList.length - 1]?.АдресЭП;
        } else {
          email = emailList?.АдресЭП;
        }
      } else if (contactInfo && typeof contactInfo === 'object') {
        if (contactInfo.Тип === 'Телефон') {
          phone = contactInfo?.НомерТелефона;
        }

        if (contactInfo.Тип === 'АдресЭлектроннойПочты') {
          email = contactInfo?.АдресЭП;
        }
      }
    }
