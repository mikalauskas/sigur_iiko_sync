  ...
  return data
    ...
    .map((el) => {
      ...
      if (Object.hasOwn(el, 'phone') && el.phone) {
        ...
        if (phone && phone.isValid() && phone.country === 'RU') {
      ...
      if (Object.hasOwn(el, 'email') && el.email) {
        ...
        if (el.email) el.email = el.email.toLowerCase();
      }
      ...
      if (el.contract_code && Object.hasOwn(el, 'contract_code')) {
      ...
      if (el.customer_email && Object.hasOwn(el, 'customer_email')) {
        ...
        if (el.customer_email) el.customer_email = el.customer_email.toLowerCase();
      }
...
const create1cJsonData = async () => {
  ...
  try {
    const studentList = await fetchData(
      'Catalog_Студенты?$select=Ref_Key,IsFolder,DeletionMark,ФизЛицо_Key,Code,Description,Специальность_Key,АнкетаАбитуриента_Key,ИдБитрикс,Логин,Пароль,ПрограммаСПО_Key',
      '1c_students.json',
      studentModel,
    );
    ...
    const personList = await fetchData(
      'Catalog_ФизическиеЛица?$select=Ref_Key,IsFolder,СтраховойНомерПФР,КонтактнаяИнформация',
      '1c_persons.json',
      personModel,
    );
    ...
    const groupList = await fetchData(
      'Catalog_УчебныеГруппы?$select=Ref_Key,IsFolder,Статус,ПрограммаСПО_Key,РабочийУчебныйПлан_Key,Code,Description,ГодПоступления,ФормаОбучения,ДатаОкончанияОбучения',
      '1c_groups.json',
      groupModel,
    );
    const contractList = await fetchData(
      'Document_КонтрактНаОбучение?$select=Ref_Key,Абитуриент_Key,Специальность_Key,ПрограммаОбучения_Key,Number,bma_РегистрационныйНомер,Date,ДатаОкончанияКонтракта,ЗаказчикОнЖе,Заказчик,ТелефонЗаказчика,ЭлектроннаяПочтаЗаказчика,ЭлектроннаяПочтаЗаказчика,ЗаказчикЮридическоеЛицо,СтоимостьОбучения,умкГрафикОплаты',
      '1c_contracts.json',
      contractModel,
    );
