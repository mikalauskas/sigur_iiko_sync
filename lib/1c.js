...
const groupModel = (data) => {
    ...
    return data.map(el => {
        ...
        if (properties['d:IsFolder']._text === 'true' || properties['d:Статус']._text !== 'Учится') return null;
...
const main = async () => {
    try {
        ...
        let groupList = await fetchData('Catalog_УчебныеГруппы', '1c_groups.json', groupModel);
        let contractList = await fetchData('Document_КонтрактНаОбучение', '1c_contracts.json', contractModel);
        ...
        await fs.writeFile('1c_merged.json', JSON.stringify(mergedContract, null, 2));
    } catch (err) {
        console.error(err);
    }
}
