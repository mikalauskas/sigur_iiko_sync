...
const endpoint = process.env.WAZZUP_ENDPOINT;
 ...
 */
const sendMessage = async (data, sendSms = false) => {
  ...
  const url1 = new URL('message.php', endpoint);
  ...
  try {
    ...
    if (!response.ok) {
      ...
      console.log(`wazzup: sent message: ${body.phone}: ${body.message}`);
      return await response.json();
    }
  } catch (err) {
    console.error(err.message);
  ...
  if (!result?.messageId) {
    ...
    await utils.delay(Math.floor(Math.random() * (186000 - 180000 + 1)) + 180000);
