...
module.exports = class Iiko {
  ...
  async syncIiko(CUsers = [], sigurUsers = []) {
      ...
      if (iikoUser?.id) {
        ...
        if (cardsToRemove.length > 0) {
          for (const card of cardsToRemove) {
            if (card?.number) {
              console.log(
                `iiko: remove card ${user.phone} ${user.fullname} ${card.number}`,
              );
        ...
        if (userCard.length < 1) {
          console.log(`iiko: add card ${user.phone} ${user.fullname} ${user.sigur_key}`);
        ...
        if (createdUser?.id) {
          console.log(
            `iiko: addusertocategory ${user.phone} ${user.fullname} ${createdUser.id}`,
          );
    ...
    console.log(`Total users in iiko: ${iikoUsersCount}`);
